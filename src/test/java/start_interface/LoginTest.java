...
class LoginTest {
    ...
    @Test
    void checkEmailFormat_acceptsDiverseValidPatterns() {
        LoginInteractor interactor = new LoginInteractor(new LoginPresenterSpy(), new InMemoryUserDataAccessObject());

        assertTrue(interactor.checkEmailFormat("user.name+alias@example-domain.ca"));
    }

    @Test
    void checkEmailEmpty_treatsNullAsEmpty() {
        LoginInteractor interactor = new LoginInteractor(new LoginPresenterSpy(), new InMemoryUserDataAccessObject());

        assertTrue(interactor.checkEmailEmpty(null));
    }

    @Test
    void checkPasswordEmpty_treatsNullAsEmpty() {
        LoginInteractor interactor = new LoginInteractor(new LoginPresenterSpy(), new InMemoryUserDataAccessObject());

        assertTrue(interactor.checkPasswordEmpty(null));
    }

    @Test
    void checkValidUser_existingUserReturnsTrueEvenWithWrongPassword() {
        InMemoryUserDataAccessObject repository = new InMemoryUserDataAccessObject();
        repository.addUser(createUser("paul@example.com", "password123"));
        LoginInteractor interactor = new LoginInteractor(new LoginPresenterSpy(), repository);

        assertTrue(interactor.checkValidUser("paul@example.com", "different"));
    }

    @Test
    void checkValidPassword_existingUserReturnsFalseWithWrongPassword() {
        InMemoryUserDataAccessObject repository = new InMemoryUserDataAccessObject();
        repository.addUser(createUser("paul@example.com", "password123"));
        LoginInteractor interactor = new LoginInteractor(new LoginPresenterSpy(), repository);

        assertFalse(interactor.checkValidPassword("paul@example.com", "different"));
    }
